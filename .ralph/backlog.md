...
[x] Update CLAUDE.md to document the release-based deployment structure. Add the new `preserve` and `keepReleases` config fields, show the new directory layout with `releases/` + `current` + `preserve/`, document the new `TOSS_ENV_DIR` environment variable, and explain that old releases are kept for production but not for previews. Update examples for `TOSS_RELEASE_DIR` and `TOSS_PROD_DIR` to point at `current/` where relevant.
