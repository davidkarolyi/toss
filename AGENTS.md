...
- `prodDomain` (optional): explicit hostname for prod (e.g., `example.com` for apex)
- `prodAliases` (optional): additional hostnames for prod (e.g., `www.example.com`)
- `prodAliasRedirect` (optional): if true, aliases redirect to the primary prod hostname
...
`prodDomain` only affects prod; previews still use `domain`.

`app` and `domain` are separate inputs, but toss avoids duplicating the app name when the domain is already app-scoped. Example: app `webref` + domain `webref.ai` → `prod.webref.ai` (not `prod.webref.webref.ai`). Use `prodDomain` if you want prod at the apex (e.g., `webref.ai`).
...
Prod hostname (optional): example.com
Prod aliases (optional): www.example.com
Start command: npm start
Deploy commands (deployScript): npm ci && npm run build
Persistent dirs (optional): data, uploads
...
1. If you set a domain, add DNS records:
   ...
   If you set `prodDomain`, also add an A record for that hostname (and any aliases like `www`).
...
If `prodDomain` and `prodAliases` are set with `prodAliasRedirect: true`, toss will add redirect blocks for aliases (e.g., `www` → apex) and keep the primary hostname on the reverse proxy block.
...
If a domain is configured, preview URLs use `pr-<number>.<app-domain>` where `app-domain` is either `<app>.<domain>` or the domain itself if it already starts with `<app>.`.
